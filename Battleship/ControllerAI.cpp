...
Point* CControllerAI::GetAdjacentCoords(Point& _point)
{
	Point points[4] =
	{
		{_point.x - 1, _point.y},
...
Point& CControllerAI::RotateDirection90(Point& _direction)
{
	short x = -_direction.y;
	short y = _direction.x;
	_direction.x = x;
	_direction.y = y;
	return _direction;
}

Point& CControllerAI::RandomDirection(Point& _direction)
{
	for (int i = rand() % 4; i > 0; --i)
	{
		RotateDirection90(_direction); // random direction
	}
	return _direction;
}

void CControllerAI::ReverseHistory(Point** _history, short _length)
{
	for (int i = 0; i < _length / 2; ++i)
	{
		Point* temp = _history[i];
		_history[i] = _history[_length - i - 1];
		_history[_length - i - 1] = temp;
	}
}